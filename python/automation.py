...
def travel_animation_check():

    x, y = int(constants.ANIMATION_CHECK_PIXEL_WIDTH_RATIO * screen_width), int(constants.ANIMATION_CHECK_PIXEL_HEIGHT_RATIO * screen_height)


    # Wait for the black loading screen to begin
    while True:
    ...
    # Wait for black loading screen to finish
    while True:
        pixel = pyautogui.pixel(x, y)
        if pixel != (0, 0, 0):
            print("Loading screen animaton done.")
            break
...
# This is to ensure an image is in the form of a numpy array as that is what opencv accepts. It does not accept other formats
...
def is_image_similar(screenshot, template, threshold=0.85):
...
def get_image_confidence(screenshot, template):
...
def portal_page_distance_check_and_command_send(zone):
    ...
    for folder in folders:
        for file in folder.iterdir(): # Creates iterable containing all files in that folder
            ...
            command_name = portal_name.replace("_", " ").title().strip() # This basically converts "green_hill" into "Green Hill"
            ...
            confidence = get_image_confidence(portal_image, image)
            if confidence > best_confidence[0]:
                best_confidence = [confidence, command_name]
            else:
                continue

            
    page_distance = constants.ZONES_PAGE_NUMBER[zone] - constants.ZONES_PAGE_NUMBER[best_confidence[1]]
...
def is_portal_locked(zone):
    ...
    print("Checking if {zone} is locked.\n Below you can see the confidence and threshold respectively.")
    if is_image_similar(image, portal_locked) == True:
        print("Portal is locked")
        return True

    print("Checking if {zone} is unlocked.\n Below you can see the confidence and threshold respectively.")
    if is_image_similar(image, portal_unlocked) == True:
        print("Portal is unlocked")
        return False
...
def auto_run_entered_check(zone):
    ...
    print("Checking if auto run has been entered")
...
def send_command(command):
    ...
    if command in constants.ZONE_BUTTON_COORDINATES:
        ...
        zone_with_underscore = command.replace(" ", "_")
        send_command(f"{zone_with_underscore}_Portal")
            
    # Wait for loading screen animations to finish
    if command in constants.ZONE_BUTTON_COORDINATES:
        travel_animation_check()
